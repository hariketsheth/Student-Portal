...
else if ((strlen($_POST['uname']) != 0) || (strlen($_POST['pass']) != 0) ) {
...
if ($_POST['uname']== $fetch_info['username'] && $_POST['pass']==$fetch_info['codekey'] && $fetch_info['role_1']=="active"){
    ...
    swal.fire( 'Hariom <?php echo $fetch_info['name']; ?>' ,  
	  ...
	  'success' )
.then((value) => {
	 location.href='welcome/account.php'
	 });
     </script>
     <?php
}
else if ($_POST['uname']== $fetch_info['username'] && $_POST['pass']==$fetch_info['codekey'] && $fetch_info['role_1']=="inactive"){
    $_SESSION['uname']=$_POST['uname'];
    $_SESSION['last_login_timestamp'] = time(); 
...
else if ( ($_POST['uname']==$fetch_info['username'] && $_POST['pass']!=$fetch_info['codekey']) && ((strlen($_POST['uname']) != 0) &&( (strlen($_POST['pass']) >= 6) ) )) {
?>
 <script>
...
else if ( ($_POST['uname']==$fetch_info['username'] && $_POST['pass']!=$fetch_info['codekey']) && ((strlen($_POST['pass']) > 0) &&( (strlen($_POST['pass']) < 6) ) )) {
    ?>
    

<script>Swal.fire({
  title: 'PASSWORD ERROR:',
  text: "We will not permit you to enter the Portal. If you have lost your password then you can contact us, <?php echo $fetch_info['name']; ?> !",
  icon: 'warning',
  showCancelButton: true,
...
else if ((strlen($_POST['pass']) > 0) && (strlen($_POST['uname']) == 0))
{
 ?>
 <script>Swal.fire({
...
<?php
  ...
  else if (($_POST['uname']!= $fetch_info['username']) && (strlen($_POST['pass']) != 0)  ){
